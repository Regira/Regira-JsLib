...
— whose **named** exports include everything below (the barrel omits the ones marked *(subpath/file only)*).
...
export interface CountdownValues { days: number; hours: number; minutes: number; seconds: number; }
...
export declare const addImageToCanvas: (canvas: HTMLCanvasElement | null | undefined, img: HTMLImageElement, { top, left }?: { top?: number; left?: number; }) => HTMLCanvasElement;   // file only
...
export declare const resizeByScale: (img: HTMLImageElement, scale: number, { quality, type }?: { quality?: number; type?: string; }) => Promise<HTMLImageElement>;
export declare const resize: (img: HTMLImageElement, maxSize: number | [number, number], { quality, type }?: { quality?: number; type?: string; }) => Promise<HTMLImageElement>;
