...
| Nested collections empty on the overview (List/Search)             | On complex entities the API loads navigations only when the request sends `?includes=`                                                                                        | Set `baseQueryParams: { includes: "All" }` (or the needed flags) in `config/config.ts` — List/Search send it on every request. **Complex API entities only** — a **simple** entity ignores `?includes=`, so eager-load the relation on the back-end (`e.Includes(...)`) instead                                    |
