...
Share **one** `useDragDrop` engine (from `regira_modules/vue/entities`) through the whole recursion via a prop — each level creating its own engine breaks cross-level drops. On a move: persist the edge change through the entity service first, then mirror it client-side with `tree.move(child, parent)`.
